...
BOOST_AUTO_TEST_CASE(imcw_person)
{
  using boost::gregorian::months;
  using boost::gregorian::years;
  using date = click_card::date;
  ...
  //A person that has not spend anything has a neutral balance
  {
    ...
    BOOST_CHECK(expected_euros == observed);
  }
  //A default person will always buy Winners
  {
    ...
    BOOST_CHECK(!p.will_buy_winners(date(2015,6,15)));
    BOOST_CHECK( p.will_buy_winners(date(2015,7, 1)));
    BOOST_CHECK( p.will_buy_winners(date(2016,6,30)));
    BOOST_CHECK(!p.will_buy_winners(date(2016,7, 1)));
    p.add_click_card(click_card(date(2016,6,15))); //Cannot buy Winners without ClickCard
    BOOST_CHECK( p.will_buy_winners(date(2016,7, 1)));
    BOOST_CHECK(!p.will_buy_winners(date(2017,7, 1)));
  }
  ...
  //A person buying any WinnerPackage will obtain a ClickCard that is valid for a year
  {
    ...
    company mcw;
    ;
    BOOST_CHECK(!p.has_valid_click_card(c.get_today()));
    ...
    BOOST_CHECK( p.has_valid_click_card(date(2016,6,30)));
    BOOST_CHECK(!p.has_valid_click_card(date(2016,7, 1)));
  }
  ...
  //and when all Winners are gone
  {
    ...
    BOOST_CHECK(!p.will_tranfer(c.get_today()));
    BOOST_CHECK(!p.will_tranfer(c.get_today() + boost::gregorian::years(1)));
    //Still has winners
    BOOST_CHECK(!p.will_tranfer(c.get_today() + boost::gregorian::years(1) + boost::gregorian::days(1)));
    p.get_winners().clear();
    #ifdef FIX_ISSUE_12
    BOOST_CHECK(!p.will_tranfer(c.get_today() + boost::gregorian::years(1) + boost::gregorian::days(1)));
    #endif // FIX_ISSUE_12
  ...
  //A person his/her profit is distributed over BankWallet and ShopWallet
  {
    ...
    BOOST_CHECK(expected_bank_wallet == observed_bank_wallet);
    BOOST_CHECK(expected_shop_wallet == observed_shop_wallet);
  }
}
